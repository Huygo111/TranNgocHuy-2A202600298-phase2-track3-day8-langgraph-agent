...
def _event_latency_ms(events: list[dict[str, Any]]) -> int:
    total = 0
    for event in events:
        metadata = event.get("metadata") or {}
        total += int(event.get("latency_ms", 0) or metadata.get("latency_ms", 0) or 0)
    return total
...
def metric_from_state(
    ...
    return ScenarioMetric(
        ...
        latency_ms=_event_latency_ms(events),
        errors=list(errors),
    )
